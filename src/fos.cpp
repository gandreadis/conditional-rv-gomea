...
fos_t::fos_t( int problem_index, int FOS_type )
{
	if( problem_index == 16 )
	{
		int FOS_element_size = 5;
		int num_large_blocks = (number_of_parameters+FOS_element_size-1) / FOS_element_size;
...
// MPM
fos_t::fos_t( int FOS_element_size )
{
	assert( number_of_parameters % FOS_element_size == 0 );
	for( int i = 0; i < number_of_parameters/FOS_element_size; i++ )
	{
...
fos_t::fos_t( double **covariance_matrix )
{
	...
	int length             = number_of_parameters+number_of_parameters-1;
	if( problem_index != 14 )
	{
...
fos_t::fos_t( const std::map<int,std::set<int>> &variable_interaction_graph, int max_clique_size, bool include_cliques_as_fos_elements, bool include_full_fos_element )
{
...
int *fos_t::matchFOSElements( fos_t *other )
{
	...
	int **FOS_element_similarity_matrix = (int**) Malloc((getLength()-number_of_parameters)*sizeof(int*));
	for(int i = 0; i < getLength()-number_of_parameters; i++ )
		FOS_element_similarity_matrix[i] = (int*) Malloc((getLength()-number_of_parameters)*sizeof(int));
	for(int i = 0; i < number_of_parameters; i++ )
	{
	...
	for(int i = number_of_parameters; i < getLength(); i++ )
	{
		for(int j = number_of_parameters; j < getLength(); j++ )
		{
			int a = 0;
			int b = 0;
			int matches = 0;
			...
			FOS_element_similarity_matrix[i-number_of_parameters][j-number_of_parameters] = (int) 10000*(2.0*matches/(other->sets[i].size()+sets[j].size()));
		}
	}

	int *hungarian_permutation = hungarianAlgorithm(FOS_element_similarity_matrix, getLength()-number_of_parameters);
	for(int i = 0; i < getLength()-number_of_parameters; i++ )
