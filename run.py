...
def main():
    ...
    parser.add_argument('--distribution-multiplier-decrease', type=float, default=DEFAULT_DISTRIBUTION_MULTIPLIER_DECREASE)
    parser.add_argument('--st-dev-threshold', type=float, default=DEFAULT_ST_DEV_THRESHOLD)
    parser.add_argument('-r', '--random-seed', type=int, default=DEFAULT_RANDOM_SEED)
    ...
    parser.add_argument('-x', '--lower-init-bound', type=float, default=DEFAULT_LOWER_INIT_BOUND)
    parser.add_argument('-y', '--upper-init-bound', type=float, default=DEFAULT_UPPER_INIT_BOUND)
    parser.add_argument('-v', '--vtr', type=float, default=DEFAULT_VTR)
    parser.add_argument('-a', '--rotation-angle', type=float, default=DEFAULT_ROTATION_ANGLE)

    parser.add_argument('-e', '--max-num-evaluations', type=float, default=DEFAULT_MAX_NUM_EVALUATIONS)
    parser.add_argument('-t', '--max-num-seconds', type=float, default=DEFAULT_MAX_NUM_SECONDS)
    parser.add_argument('--max-no-improvement-stretch', type=int, default=DEFAULT_MAX_NO_IMPROVEMENT_STRETCH)
    parser.add_argument('-f', '--fitness-variance-tolerance', type=float, default=DEFAULT_FITNESS_VARIANCE_TOLERANCE)
